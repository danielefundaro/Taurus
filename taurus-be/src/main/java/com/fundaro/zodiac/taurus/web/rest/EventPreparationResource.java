...
public class EventPreparationResource {
    ...
    public EventPreparationResource(EventPreparationService service) {
    ...
    @PreAuthorize("hasAnyAuthority('ROLE_SUPER_ADMIN','ROLE_ADMIN')")
    public View configure(@PathVariable Long eventId, @Valid @RequestBody Configuration request, AbstractAuthenticationToken token) {
        return service.configure(eventId, request, token);
    }

    @PutMapping("/program")
    @PreAuthorize("hasAnyAuthority('ROLE_SUPER_ADMIN','ROLE_ADMIN','ROLE_ARCHIVIST')")
    public View program(@PathVariable Long eventId, @Valid @RequestBody ProgramRequest request, AbstractAuthenticationToken token) {
        return service.replaceProgram(eventId, request, token);
    }

    @PutMapping("/materials")
    @PreAuthorize("hasAnyAuthority('ROLE_SUPER_ADMIN','ROLE_ADMIN')")
    @RequiresTenantFeature({TenantFeature.EVENT_PREPARATION, TenantFeature.INVENTORY})
    public View materials(@PathVariable Long eventId, @Valid @RequestBody MaterialsRequest request, AbstractAuthenticationToken token) {
        return service.replaceMaterials(eventId, request, token);
    }

    @PostMapping("/materials/{materialId}/confirm")
    @PreAuthorize("hasAnyAuthority('ROLE_SUPER_ADMIN','ROLE_ADMIN')")
    @RequiresTenantFeature({TenantFeature.EVENT_PREPARATION, TenantFeature.INVENTORY})
    public View confirmMaterial(@PathVariable Long eventId, @PathVariable Long materialId, AbstractAuthenticationToken token) {
        return service.confirmMaterial(eventId, materialId, token);
    }
    ...
    @PreAuthorize("hasAnyAuthority('ROLE_SUPER_ADMIN','ROLE_ADMIN')")
    public View confirmPresence(@PathVariable Long eventId, AbstractAuthenticationToken token) {
        return service.confirmPresence(eventId, token);
    }
