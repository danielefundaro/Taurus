...
export class CalendarFeedService {
    ...
    create(request: CalendarFeedCreate, admin = false): Observable<CalendarFeedSecret> {
        return this.http.post<CalendarFeedSecret>(this.url(admin), request);
    }
    rotate(id: string, admin = false, idempotencyKey: string = crypto.randomUUID()): Observable<CalendarFeedSecret> {
        return this.http.post<CalendarFeedSecret>(`${this.url(admin)}/${id}/rotate`, { idempotencyKey });
    }
