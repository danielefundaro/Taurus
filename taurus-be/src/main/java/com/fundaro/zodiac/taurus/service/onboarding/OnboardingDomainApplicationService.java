...
public class OnboardingDomainApplicationService {
    ...
    public void apply(Long jobId, List<OnboardingImportRow> rows, Map<Long, String> keycloakIds, String tenantCode, String actor) {
        ...
        for (OnboardingImportRow row : scoped(rows, OnboardingSection.INSTRUMENTS)) {
            ...
            if (entity == null) {
                ...
                audit(entity, actor, now);
                entity.setName(value(row, "nome"));
                entity.setDescription(blankToNull(value(row, "descrizione")));
                entity = instruments.save(entity);
                instrumentNames.put(key(entity.getName()), entity);
            }
            instrumentRefs.put(value(row, "riferimento").toUpperCase(Locale.ROOT), entity);
            applied(row);
        }
        for (Map.Entry<String, Instruments> entry : instrumentNames.entrySet())
            instrumentRefs.putIfAbsent(entry.getKey().toUpperCase(Locale.ROOT), entry.getValue());
        ...
        for (OnboardingImportRow row : scoped(rows, OnboardingSection.USERS)) {
            if (row.getAction() == OnboardingRowAction.SKIP) {
                skipped(row);
                continue;
            }
            String keycloakId = Objects.requireNonNull(keycloakIds.get(row.getId()), "Prepared identity missing");
            UserIdentity identity = identities.findByKeycloakId(keycloakId).orElseGet(() -> {
                UserIdentity created = new UserIdentity();
                created.setKeycloakId(keycloakId);
                created.initializeAudit(actor);
                return identities.save(created);
            });
            Users entity = new Users();
            audit(entity, actor, now);
            entity.setName(value(row, "nome"));
            entity.setLastName(value(row, "cognome"));
            entity.setEmail(value(row, "email"));
            entity.setKeycloakId(keycloakId);
            entity.setUserIdentity(identity);
            entity.setActive("SI".equals(value(row, "attivo")));
            if (!value(row, "data_nascita").isBlank())
                entity.setBirthDate(java.sql.Date.valueOf(value(row, "data_nascita")));
            ...
            entity.setInstruments(split(value(row, "strumenti")).stream().map(ref -> instrumentRefs.get(ref.toUpperCase(Locale.ROOT))).filter(Objects::nonNull).toList());
            users.save(entity);
            ...
            membership.setId(new TenantUserMembershipId(tenant.getId(), identity.getId()));
            membership.setActive(true);
            membership.setJoinedAt(ZonedDateTime.now());
            membership.setLeftAt(null);
            membership.initializeAudit(actor);
        ...
        for (OnboardingImportRow row : scoped(rows, OnboardingSection.INVENTORY)) {
            ...
            item.setInventoryNumber(value(row, "numero_inventario"));
            item.setName(value(row, "nome"));
            item.setDescription(blankToNull(value(row, "descrizione")));
            item.setTotalQuantity(integer(row, "quantita_totale"));
            item.setEstimatedUnitValue(decimalOrNull(row, "valore_unitario_stimato"));
            item.setCurrency(blankToNull(value(row, "valuta")));
            item.setConditionStatus(InventoryCondition.valueOf(value(row, "condizione")));
            item.setConditionNotes(blankToNull(value(row, "note_condizione")));
        ...
        for (OnboardingImportRow row : scoped(rows, OnboardingSection.CATEGORIES)) {
            ...
            if (category == null) {
                ...
                category.setName(value(row, "nome"));
                category.setDescription(blankToNull(value(row, "descrizione")));
                category.setDirection(FinancialCategoryDirection.valueOf(value(row, "direzione")));
                category.setActive(true);
                category.setSystemDefined(false);
                category.setDisplayOrder(value(row, "ordine").isBlank() ? ++categoryOrder : integer(row, "ordine"));
                category = categories.save(category);
                categoryNames.put(key(category.getName()), category);
            }
            applied(row);
        }

        Map<String, FinancialAccount> accountRefs = new HashMap<>();
        Map<String, FinancialAccount> accountNames = accounts.findAllByDeletedFalseAndActiveTrueOrderByDisplayOrderAscNameAsc().stream().collect(Collectors.toMap(a -> key(a.getName()), a -> a, (x, y) -> x));
        ...
        for (OnboardingImportRow row : scoped(rows, OnboardingSection.ACCOUNTS)) {
            ...
            if (account == null) {
                ...
                account.initializeAudit(actor);
                account.setName(value(row, "nome"));
                account.setDescription(blankToNull(value(row, "descrizione")));
                account.setAccountType(FinancialAccountType.valueOf(value(row, "tipo")));
                account.setCurrency(value(row, "valuta"));
                account.setIban(blankToNull(value(row, "iban")));
                account.setBankName(blankToNull(value(row, "banca")));
                account.setActive(true);
                account.setDisplayOrder(value(row, "ordine").isBlank() ? ++accountOrder : integer(row, "ordine"));
                account = accounts.save(account);
                accountNames.put(key(account.getName()), account);
            }
            accountRefs.put(value(row, "riferimento").toUpperCase(Locale.ROOT), account);
            applied(row);
        }
        ...
        for (OnboardingImportRow row : scoped(rows, OnboardingSection.OPENING_BALANCES)) {
            BigDecimal signed = decimal(row, "importo");
            if (signed.signum() == 0) {
                skipped(row);
                continue;
            }
            String reference = value(row, "conto");
            FinancialAccount account = accountRefs.get(reference.toUpperCase(Locale.ROOT));
            if (account == null) account = accountNames.get(key(reference));
            if (account == null || movements.countByAccount_IdAndDeletedFalse(account.getId()) > 0)
                throw new IllegalStateException("OPENING_BALANCE_NOT_ALLOWED");
            LocalDate date = LocalDate.parse(value(row, "data"));
            AccountingYear year = years.findByYearAndDeletedFalse(date.getYear()).orElseGet(() -> {
                AccountingYear created = new AccountingYear();
                created.initializeAudit(actor);
                created.setYear(date.getYear());
                created.setStartDate(LocalDate.of(date.getYear(), 1, 1));
                created.setEndDate(LocalDate.of(date.getYear(), 12, 31));
                created.setStatus(AccountingYearStatus.OPEN);
            ...
            movement.setDirection(signed.signum() > 0 ? FinancialDirection.INCOME : FinancialDirection.EXPENSE);
            movement.setNature(FinancialMovementNature.OPENING);
            movement.setBookingDate(date);
            movement.setValueDate(date);
            movement.setAmount(signed.abs());
            movement.setCurrency(account.getCurrency());
            movement.setDescription("Saldo iniziale");
        ...
        for (OnboardingImportSection summary : summaries.findAllByJob_IdOrderBySectionAsc(jobId)) {
            summary.setApplied(Math.toIntExact(applied.getOrDefault(summary.getSection(), 0L)));
    ...
    private static void applied(OnboardingImportRow row) {
        row.setStatus(OnboardingRowStatus.APPLIED);
    }

    private static void skipped(OnboardingImportRow row) {
        row.setStatus(OnboardingRowStatus.SKIPPED);
    }

    private static String value(OnboardingImportRow row, String key) {
        return Objects.toString(row.getNormalizedPayload().get(key), "");
    }

    private static int integer(OnboardingImportRow row, String key) {
        return Integer.parseInt(value(row, key));
    }

    private static BigDecimal decimal(OnboardingImportRow row, String key) {
        return new BigDecimal(value(row, key));
    }

    private static BigDecimal decimalOrNull(OnboardingImportRow row, String key) {
        return value(row, key).isBlank() ? null : decimal(row, key);
    }
    ...
    private static void audit(CommonFieldsOpenSearch entity, String actor, Date now) {
        entity.setDeleted(false);
        entity.setInsertBy(actor);
        entity.setEditBy(actor);
        entity.setInsertDate(now);
        entity.setEditDate(now);
        entity.setEntityVersion(0L);
    }
}
