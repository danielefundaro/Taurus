...
public class CalendarEventsServiceImpl implements CalendarEventsService {
    ...
    public CalendarEventsServiceImpl(com.fundaro.zodiac.taurus.service.CalendarEventsService delegate) {
    ...
    public Optional<CalendarEventsDTO> findOne(Long id, AbstractAuthenticationToken token) {
        return delegate.findOne(id, token).filter(dto -> getVisibleStates().contains(dto.getState())).map(this::mask);
    }
    ...
    private CalendarEventsDTO mask(CalendarEventsDTO dto) {
        dto.setFee(null);
        dto.setCosts(null);
        return dto;
    }
