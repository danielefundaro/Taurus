...
class CalendarFeedMigrationIT {
    ...
    @Autowired
    TenantSchemaProvisioningService provisioning;
    @Autowired
    TenantSchemaNameResolver names;
    @Autowired
    JdbcTemplate jdbc;
    ...
    @AfterAll
    void cleanup() {
        provisioning.dropSchema(tenant);
    }

    @Test
    void provisionsGlobalRegistryAndCompleteTenantSchema() {
