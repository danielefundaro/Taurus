...
public class OnboardingImportService {
    ...
    @Transactional(readOnly = true) public Page<OnboardingDtos.Job> list(Pageable pageable, AbstractAuthenticationToken token) { tenant(token); return jobs.findAllByDeletedFalse(normalize(pageable, Sort.by(Sort.Direction.DESC, "insertDate"))).map(this::dto); }
    ...
    @Transactional(readOnly = true) public Page<OnboardingDtos.Issue> issues(Long id, OnboardingIssueSeverity severity, OnboardingSection section, Pageable pageable, AbstractAuthenticationToken token) {
        tenant(token); required(id); Pageable safe = normalize(pageable, Sort.by(Sort.Direction.ASC, "rowNumber").and(Sort.by(Sort.Direction.ASC, "id"))); Page<OnboardingImportIssue> result = severity != null && section != null ? issues.findAllByJob_IdAndSeverityAndSection(id, severity, section, safe) : severity != null ? issues.findAllByJob_IdAndSeverity(id, severity, safe) : section != null ? issues.findAllByJob_IdAndSection(id, section, safe) : issues.findAllByJob_Id(id, safe); return result.map(this::dto);
    }
    ...
    private Pageable normalize(Pageable p, Sort fallback) { return PageRequest.of(Math.max(0, p.getPageNumber()), Math.min(100, Math.max(1, p.getPageSize())), p.getSort().isSorted() ? p.getSort() : fallback); }
