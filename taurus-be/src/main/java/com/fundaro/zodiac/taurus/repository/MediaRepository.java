    ...
    @Query(
        value = """
            SELECT NOT EXISTS (
                ...
                UNION ALL
                SELECT 1 FROM inventory_issue_photo WHERE media_asset_id = :id AND deleted = FALSE
                UNION ALL
    ...
    @Query(
        value = """
            SELECT EXISTS (
                ...
                UNION ALL SELECT 1 FROM inventory_issue_photo WHERE media_asset_id = :id AND deleted = FALSE
            )
            """,
        nativeQuery = true
    )
