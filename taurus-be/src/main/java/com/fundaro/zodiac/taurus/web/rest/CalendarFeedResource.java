...
public class CalendarFeedResource {
    ...
    @PostMapping("/{id}/rotate") public SecretFeed rotate(@PathVariable UUID id, @Valid @RequestBody RotateRequest request, AbstractAuthenticationToken auth) { return service.rotate(id, request, false, auth); }
    ...
    @DeleteMapping("/{id}/record") @ResponseStatus(HttpStatus.NO_CONTENT) public void deleteRevoked(@PathVariable UUID id, AbstractAuthenticationToken auth) { service.deleteRevoked(id, false, auth); }
}
