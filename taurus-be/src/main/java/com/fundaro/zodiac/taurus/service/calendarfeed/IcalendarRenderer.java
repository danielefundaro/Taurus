...
public class IcalendarRenderer {
    ...
    public IcalendarRenderer(ApplicationProperties properties) {
        refresh = properties.getCalendarFeed().getSuggestedRefresh();
    }
    ...
    public byte[] render(RenderCalendar calendar) {
        ...
        line(out, "PRODID:-//Zodiac Taurus//Calendar Feed 1.0//IT");
        line(out, "CALSCALE:GREGORIAN");
        line(out, "METHOD:PUBLISH");
        line(out, "X-WR-CALNAME:" + text(calendar.name()));
        ...
        try {
            new CalendarBuilder().build(new ByteArrayInputStream(result));
        } catch (Exception exception) {
            throw new IllegalStateException("Invalid iCalendar projection", exception);
        }
        return result;
    }

    private void event(ByteArrayOutputStream out, RenderEvent e, CalendarFeedDetailLevel detail) {
        if (e.startAt() == null || e.endAt() == null || !e.endAt().isAfter(e.startAt()))
            throw new IllegalStateException("Invalid calendar event interval");
        line(out, "BEGIN:VEVENT");
        line(out, "UID:urn:uuid:" + e.uid());
        line(out, "DTSTAMP:" + UTC.format(e.modifiedAt()));
        line(out, "LAST-MODIFIED:" + UTC.format(e.modifiedAt()));
        line(out, "SEQUENCE:" + e.sequence());
        line(out, "DTSTART:" + UTC.format(e.startAt()));
        line(out, "DTEND:" + UTC.format(e.endAt()));
        line(out, "SUMMARY:" + text(e.summary()));
        if (!e.cancelled() && e.location() != null && !e.location().isBlank())
        ...
        if (!e.cancelled() && detail == CalendarFeedDetailLevel.STANDARD) {
            String description = plain(e.description());
            if (!description.isBlank()) line(out, "DESCRIPTION:" + text(description));
    ...
    private static String safeUri(String value) {
        return CONTROL.matcher(value).replaceAll("").replace(" ", "%20");
    }
    ...
    private static void line(ByteArrayOutputStream out, String value) {
        ...
        while (offset < value.length()) {
            ...
            int limit = 75;
            if (bytesOnLine + size > limit) {
                out.writeBytes("\r\n ".getBytes(StandardCharsets.US_ASCII));
                bytesOnLine = 1;
            }
            out.writeBytes(part.getBytes(StandardCharsets.UTF_8));
            bytesOnLine += size;
