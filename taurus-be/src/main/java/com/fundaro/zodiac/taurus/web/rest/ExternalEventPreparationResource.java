...
public class ExternalEventPreparationResource {
    ...
    public ExternalEventPreparationResource(EventPreparationService service) {
    ...
    public ResponseEntity<View> get(@PathVariable Long eventId, AbstractAuthenticationToken token) {
        return ResponseEntity.ok().cacheControl(CacheControl.noStore()).body(service.getPersonal(eventId, token, true));
    }
}
