...
public class CalendarFeedTombstoneScheduler {
    ...
    private final CalendarEventFeedTombstoneRepository repository;

    public CalendarFeedTombstoneScheduler(TenantSchemaRegistry schemas, TenantTransactionExecutor transactions, CalendarEventFeedTombstoneRepository repository) {
    ...
    public void cleanup() {
        for (String tenant : schemas.findActiveTenantCodes())
            transactions.execute(tenant, () -> repository.deleteByExpiresAtBefore(Instant.now()));
    }
}
